...
select student_no  as "학번" , student_name as "이름",to_char(entrance_date,'yyyy-mm-dd') as "입학년도"     --entrance_date 
from tb_student
where
...
select 
    professor_name  "교수이름"
        ,trunc( months_between( sysdate,  to_date( '19' || substr( professor_ssn, 1, 6 ), 'YYYYMMDD')) / 12)
from
        tb_professor
where
...
이름이 찍히도록 핚다. (성이 2 자인 경우는 교수는 없다고 가정하시오)
*/
select 
...
*/
select 
to_char(to_date('99/10/11', 'yy/mm/dd' ),'yyyy')--  2099(지금세기)
,to_char(to_date('49/10/11', 'yy/mm/dd' ),'yyyy')-- 2049(이전세기)
,to_char(to_date('99/10/11', 'rr/mm/dd' ),'yyyy')-- 1999(이전세기)
,to_char(to_date('49/10/11', 'rr/mm/dd' ),'yyyy') -- 2049 (지금세기)
from dual;
...
select 
        department_no "학과코드명 "
        ,sum( case when absence_yn = 'Y' then 1 else 0 end )  "휴학생 수"           --then  1 else 0 (기본값)
        from tb_student
         group by department_no
         order by department_no;
...
select student_name "동일이름",count(*)" 동명인"
        from tb_student
