...
public class MemberService {
	...
	public int newEmail(String email, Member member) {
		Connection conn 	=getConnection()	;
		int result =memberDao.newPassword(conn, email, member);
		if(result > 0) commit(conn);
		else rollback(conn);
		close(conn);
		return result;
	...
	public int newPhone(String phone, Member member) {
		Connection conn 	=getConnection()	;
		int result =memberDao.newPassword(conn, phone, member);
		if(result > 0) commit(conn);
		else rollback(conn);
		close(conn);
		return result;
	...
	public int newAddress(String address, Member member) {
		Connection conn 	=getConnection()	;
		int result =memberDao.newPassword(conn, address, member);
		if(result > 0) commit(conn);
		else rollback(conn);
		close(conn);
		return result;
